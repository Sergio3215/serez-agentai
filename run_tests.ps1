# ── serez-agentai Test Runner ────────────────────────────────────────────────
# Usage: .\run_tests.ps1 [-sz <ruta>] [-filter <str>]
# CWD = serez-agentai/ → import "src/tokenizer" etc. resuelven localmente.
# Nota: tests de tokenizer y tools NO requieren import "serez-ai".
...
Write-Host "serez-agentai tests ($($tests.Count)) — sz: $sz`n" -ForegroundColor Cyan
$pass = 0; $fail = 0
